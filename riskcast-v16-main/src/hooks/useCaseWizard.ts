...
import { loadDomainCaseFromStorage, saveDomainCaseToStorage } from '@/domain';
...
export function useCaseWizard(initialStep: WizardStep = 'input') {
  ...
  useEffect(() => {
    try {
      ...
      if (domainCase) {
        setCaseData(domainCase);
      }
    } catch (e) {
      console.warn('[useCaseWizard] Failed to load case data from storage:', e);
    }
  }, []);
  ...
  useEffect(() => {
    if (caseData) {
      saveDomainCaseToStorage(caseData);
    }
  }, [caseData]);
  ...
  const handleSaveDraft = useCallback(() => {
    if (caseData) {
      ...
      setCaseData(updated);
      saveDomainCaseToStorage(updated);
