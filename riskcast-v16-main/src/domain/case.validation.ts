...
export function validateDomainCase(caseData: Partial<DomainCase>): {
  ...
  const allowedModes = ['AIR', 'SEA', 'ROAD', 'RAIL', 'MULTIMODAL'];
  if (!caseData.transportMode || !allowedModes.includes(String(caseData.transportMode))) {
    issues.push({
  ...
  // Treat zero or negative cargoValue as invalid/missing for critical validation
  ...
  // Party validation (warnings, not critical for basic validation)
  if (!caseData.seller?.company || caseData.seller.company.trim() === '') {
    issues.push({
      id: 'SELLER_COMPANY_REQUIRED',
      severity: 'warning',
      message: 'Seller company name is recommended',
      detail: 'Please specify seller company for complete documentation',
  ...
  if (!caseData.seller?.email || caseData.seller.email.trim() === '') {
    issues.push({
      ...
      severity: 'warning',
      message: 'Seller email is recommended',
      detail: 'Please specify seller email for notifications',
  ...
  if (!caseData.buyer?.company || caseData.buyer.company.trim() === '') {
    issues.push({
      ...
      severity: 'warning',
      message: 'Buyer company name is recommended',
      detail: 'Please specify buyer company for complete documentation',
  ...
  if (!caseData.buyer?.email || caseData.buyer.email.trim() === '') {
    issues.push({
      ...
      severity: 'warning',
      message: 'Buyer email is recommended',
      detail: 'Please specify buyer email for notifications',
