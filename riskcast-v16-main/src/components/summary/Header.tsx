...
import { AiAdvisorTrigger } from '../AiAdvisorDock';

interface HeaderProps {
  saveState: SaveState;
  lastSaved: Date;
  // Optional action handlers (if provided, actions will be shown in header)
  actions?: {
    onBack: () => void;
    onSaveDraft: () => void;
    onRunAnalysis: () => void;
...
export function Header({ saveState, lastSaved, actions }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-[100] h-[72px] bg-[#0a1628]/80 backdrop-blur-sm border-b border-white/10">
      <div className="h-full px-12 flex items-center justify-between">
        ...
        {/* Right Actions */}
        <div className="flex items-center gap-3">
          {/* Action Buttons (Back/Save/Run Analysis) - Moved from footer */}
          {actions && (
            <HeaderActions
          ...
          <div className="flex items-center gap-3 pl-3 border-l border-white/10">
            <div className="flex items-center gap-2 text-white/40 text-xs">
              ...
              <span className="hidden sm:inline">Last saved: {getTimeSince(lastSaved)}</span>
              <span className="sm:hidden">{getTimeSince(lastSaved)}</span>
            </div>
