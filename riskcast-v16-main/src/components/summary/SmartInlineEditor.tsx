...
export function SmartInlineEditor({ 
  ...
  // CRITICAL: Ensure modal (including footer with Save button) is always fully visible
  // Modal structure: Header (~80px) + Body (max-h-[50vh]) + Footer (~80px) = ~500px total
  const modalEstimatedHeight = 500; // Header + max body + footer
  const viewportHeight = typeof window !== 'undefined' ? window.innerHeight : 800;
  const maxTop = viewportHeight - modalEstimatedHeight - 20; // 20px padding from bottom
  
  const adjustedPosition = {
    x: Math.min(Math.max(position.x, 20), (typeof window !== 'undefined' ? window.innerWidth : 1920) - 440),
    y: Math.min(Math.max(position.y, 20), Math.max(20, maxTop)), // Ensure modal doesn't go below viewport
  };
  ...
  return (
    <div
      ref={containerRef}
      className="fixed z-[100] w-[420px] backdrop-blur-2xl bg-gradient-to-br from-[#0d1f35]/98 to-[#0a1628]/98 border border-white/20 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden flex flex-col"
      style={{
        left: adjustedPosition.x,
        top: adjustedPosition.y,
        maxHeight: 'calc(100vh - 40px)', // Ensure modal never exceeds viewport height minus padding
      }}
    >
      {/* Header */}
      ...
      {/* Body - CRITICAL: Reduced max-height to ensure footer (Save button) is always visible */}
      <div className="p-5 max-h-[50vh] overflow-y-auto custom-scrollbar flex-shrink">
        {renderEditor()}
