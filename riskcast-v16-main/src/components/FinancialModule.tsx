...
export const FinancialModule: React.FC<FinancialModuleProps> = ({ financial }) => {
  ...
  // Determine why loss curve is not available
  const hasLossMetrics = financial.expectedLoss > 0 || financial.var95 > 0 || financial.cvar95 > 0;
  const hasLossCurve = financial.lossCurve && financial.lossCurve.length > 0;

  if (!hasLossCurve) {
    // Show diagnostic message based on what data is available
    let diagnosticMessage = 'No loss distribution data available';
    let diagnosticReason = '';

    if (!hasLossMetrics) {
      diagnosticReason = 'Loss metrics (expected loss, VaR, CVaR) are not available. The risk engine may not have calculated financial losses for this shipment.';
    } else if (hasLossMetrics && !hasLossCurve) {
      diagnosticReason = 'Loss metrics are available, but distribution curve data is missing. The backend may not have provided loss_distribution, loss_histogram, or financial_distribution fields.';
    }
    ...
    return (
      <GlassCard>
        <div className="text-center py-12 text-white/60">
          ...
          <p className="text-sm font-medium mb-2">{diagnosticMessage}</p>
          {diagnosticReason && (
            <p className="text-xs text-white/40 max-w-md mx-auto mt-2">
              {diagnosticReason}
            </p>
          )}
          {hasLossMetrics && (
            <div className="mt-4 text-xs text-white/50">
              <p>Available metrics: Expected Loss: ${(financial.expectedLoss / 1000).toFixed(1)}K, VaR 95%: ${(financial.var95 / 1000).toFixed(1)}K</p>
            </div>
          )}
        </div>
      </GlassCard>
