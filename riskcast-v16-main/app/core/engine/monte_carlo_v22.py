...
class MonteCarloEngineV22:
    ...
    CRITICAL: Deterministic by default - generates seed from input data hash
    to ensure same input → same output for reproducibility.
    ...
    def __init__(self, n_runs: int = 10000, random_seed: Optional[int] = None):
        """
        Initialize Monte Carlo engine
        
        Args:
            n_runs: Number of simulation runs (default: 10,000)
            random_seed: Random seed for reproducibility. If None, will be generated
                        deterministically from input data in run_simulation()
        """
        self.n_runs = n_runs
    ...
    @staticmethod
    def _generate_deterministic_seed(input_data: Dict) -> int:
        """
        Generate deterministic seed from input data hash.
        
        This ensures same input → same output for reproducibility.
        Uses hash of sorted JSON representation of input data.
        
        Args:
            input_data: Dictionary containing transport, cargo, and layer_scores
        ...
        layer_scores = input_data.get('layer_scores', {})
        
        # Extract key fields that affect simulation
        seed_data = {
            'transit_time': transport.get('transit_time', 14),
            'cargo_value': cargo.get('insurance_value', 100000),
            'carrier_risk': layer_scores.get('carrier_performance', 50),
            'port_risk': layer_scores.get('port_congestion', 40),
            'weather_risk': layer_scores.get('weather_climate', 35),
            'doc_risk': layer_scores.get('documentation_complexity', 40),
            'market_risk': layer_scores.get('market_volatility', 40),
            'sensitivity': layer_scores.get('cargo_sensitivity', 40),
        }
        ...
        # Convert to 32-bit integer (numpy seed range)
    ...
    def run_simulation(self, transport: Dict, cargo: Dict, layer_scores: Dict) -> Dict:
        ...
        CRITICAL: This method is deterministic. If random_seed was not provided
        in __init__, it will generate a deterministic seed from input data to
        ...
        Returns:
            ...
            Includes 'random_seed' field for reproducibility.
        """
        ...
        if self.random_seed is None:
            ...
            deterministic_seed = self._generate_deterministic_seed(input_data)
            np.random.seed(deterministic_seed)
            used_seed = deterministic_seed
        else:
            used_seed = self.random_seed
        ...
        return {
            ...
            # Reproducibility: Save seed for traceability
