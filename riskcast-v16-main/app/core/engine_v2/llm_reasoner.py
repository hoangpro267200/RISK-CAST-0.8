...
from app.core.engine_v2.risk_driver_derivation import (
    derive_risk_drivers,
    drivers_to_dict_list,
    RiskDriver
)
...
class LLMReasoner:
    ...
    def generate_reasoning_deterministic(self, factors: Dict[str, float],
        ...
        """
        # STEP 7B.2: Driver-based explanation (not factor-based)
        # Primary explanations MUST be driver-based, not factor-based
        risk_drivers = derive_risk_drivers(
            factors=factors,
            total_risk_score=score,
            components={}  # Can pass components if available
        )
        
        # Build explanation based on drivers (engine truth), not raw factors
        explanation_parts = []
        explanation_parts.append(f"Risk assessment indicates a {profile.get('level', 'Medium')} risk level (score: {score:.1f}/100).")
        
        if risk_drivers:
            # Driver-based explanation: reference drivers by name and impact
            driver_names = [d.name for d in risk_drivers]
            if len(risk_drivers) == 1:
                explanation_parts.append(
                    f"The primary risk driver is {driver_names[0]} ({risk_drivers[0].impact:.1f}% contribution)."
                )
            elif len(risk_drivers) == 2:
                explanation_parts.append(
                    f"The primary risk drivers are {driver_names[0]} ({risk_drivers[0].impact:.1f}%) "
                    f"and {driver_names[1]} ({risk_drivers[1].impact:.1f}%)."
                )
            else:
                explanation_parts.append(
                    f"The primary risk drivers are {driver_names[0]} ({risk_drivers[0].impact:.1f}%), "
                    f"{driver_names[1]} ({risk_drivers[1].impact:.1f}%), "
                    f"and {driver_names[2]} ({risk_drivers[2].impact:.1f}%)."
                )
            
            # Add driver-specific context (secondary, supporting evidence)
            for driver in risk_drivers:
                if driver.description:
                    explanation_parts.append(driver.description)
        else:
            # Empty drivers: explain that risk is diffuse / non-dominant
            explanation_parts.append(
                "The overall risk is distributed across multiple minor factors, with no single dominant driver."
            )
            # Add factor-specific context only as secondary evidence
            if "climate" in factors and factors["climate"] > 0.6:
                explanation_parts.append("Climate conditions pose some risk, but not as a dominant driver.")
            if "network" in factors and factors["network"] > 0.6:
                explanation_parts.append("Network dependencies create some vulnerability, but not as a dominant driver.")
            if "carrier" in factors and factors.get("carrier", 0) > 0.6:
                explanation_parts.append("Carrier reliability concerns exist, but not as a dominant driver.")
        
        explanation = " ".join(explanation_parts)
        
        # Key drivers: list of driver names (for backward compatibility)
        # Order matches descending impact
        key_drivers = [driver.name for driver in risk_drivers]
