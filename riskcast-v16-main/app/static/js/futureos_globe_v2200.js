...
function initGlobe() {
  ...
  // Night texture - using NASA public domain texture
  textureLoader.load(
    ...
    undefined,
    (err) => {
      console.warn('[Globe] Failed to load Earth night texture, using day texture as fallback:', err);
      // Use day texture as fallback for night if night fails
    }
  );
  ...
  function createNetworkNode(lat, lon, r) {
    ...
    // Using MeshStandardMaterial instead of MeshBasicMaterial for emissive support
