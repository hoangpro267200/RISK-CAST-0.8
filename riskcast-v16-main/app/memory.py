"""
RISKCAST Enterprise AI - Memory System
Supports both JSON files (legacy) and MySQL (new)
"""
import os
import json
import uuid
...
if USE_MYSQL:
    # Use MySQL-based memory system
    try:
        from app.database.memory_mysql import MemorySystemMySQL
        memory_system = MemorySystemMySQL()
        print("[Memory System] Using MySQL storage")
    except ImportError as e:
        print(f"[Memory System] MySQL not available, falling back to JSON files: {e}")
        USE_MYSQL = False

if not USE_MYSQL:
    # Fallback to JSON file-based storage (legacy)
    @dataclass
    ...
    class MemorySystem:
        """Mini Memory System for RISKCAST (JSON-based)"""
    ...
    # Global memory instance (JSON-based)
